...
class subscription():
    def __init__(self, tenant_id, app_id, app_secret, subscription_id):
        ...
        self.access_token = mcazurerm.get_access_token(tenant_id, app_id, app_secret)

    # update the authentication token for this subscription
    def auth(self):
        self.access_token = mcazurerm.get_access_token(self.tenant_id, self.app_id, self.app_secret)
        return self.access_token

    # list VM Scale Sets in this subscription - names only
    def get_vmss_list(self):
        vmss_sub_list = mcazurerm.list_vmss_sub(self.access_token, self.sub_id)
        self.vmsslist = []
        self.vmssdict = {}
