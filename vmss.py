...
class vmss():
    ...
    def refresh_model(self):
        vmssmodel = mcazurerm.get_vmss(self.access_token, self.sub_id, self.rgname, self.name)
        self.model = vmssmodel
        self.capacity = vmssmodel['sku']['capacity']
        self.vmsize = vmssmodel['sku']['name']
    ...
    def scale(self, capacity):
        ...
        scaleoutput = mcazurerm.scale_vmss(self.access_token, self.sub_id, self.rgname, self.name, self.vmsize, self.tier,
                                         capacity)
        self.status = scaleoutput

    # power on all the VMs in the scale set
    def poweron(self):
        result = mcazurerm.start_vmss(self.access_token, self.sub_id, self.rgname, self.name)
        self.status = result

    def restart(self):
        result = mcazurerm.restart_vmss(self.access_token, self.sub_id, self.rgname, self.name)
        self.status = result

    # power off all the VMs in the scale set
    def poweroff(self):
        result = mcazurerm.poweroff_vmss(self.access_token, self.sub_id, self.rgname, self.name)
        self.status = result

    # stop deallocate all the VMs in the scale set
    def dealloc(self):
        result = mcazurerm.stopdealloc_vmss(self.access_token, self.sub_id, self.rgname, self.name)
        self.status = result
    ...
    def init_vm_instance_view(self):
        ...
        self.vm_instance_view = \
            mcazurerm.list_vmss_vm_instance_view(self.access_token, self.sub_id, self.rgname, self.name)

    # operations on individual VMs or groups of VMs in a scale set
    # operations on individual VMs or groups of VMs in a scale set
    def reimagevm(self, vmstring):
        result = mcazurerm.reimage_vmss_vms(self.access_token, self.sub_id, self.rgname, self.name, vmstring)
        self.status = result

    def upgradevm(self, vmstring):
        result = mcazurerm.upgrade_vmss_vms(self.access_token, self.sub_id, self.rgname, self.name, vmstring)
        self.status = result

    def deletevm(self, vmstring):
        result = mcazurerm.delete_vmss_vms(self.access_token, self.sub_id, self.rgname, self.name, vmstring)
        self.status = result

    def startvm(self, vmstring):
        result = mcazurerm.start_vmss_vms(self.access_token, self.sub_id, self.rgname, self.name, vmstring)
        self.status = result

    def restartvm(self, vmstring):
        result = mcazurerm.restart_vmss_vms(self.access_token, self.sub_id, self.rgname, self.name, vmstring)
        self.status = result

    def deallocvm(self, vmstring):
        result = mcazurerm.stopdealloc_vmss_vms(self.access_token, self.sub_id, self.rgname, self.name, vmstring)
        self.status = result

    def poweroffvm(self, vmstring):
        result = mcazurerm.poweroff_vmss_vms(self.access_token, self.sub_id, self.rgname, self.name, vmstring)
        self.status = result
